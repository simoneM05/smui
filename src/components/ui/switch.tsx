...
const useSwitchContext = () => {
  ...
  if (!context)
    throw new Error("useSwitchContext must be used within a SwitchWrapper");
  return context;
};
...
function Switch({
  ...
  VariantProps<typeof switchVariant> & { thumbClassName?: string }) {
  const context = useSwitchContext();
  const permanent = context?.permanent ?? false;
  return (
    <SwitchPrimitive.Root
      data-slot="switch"
      className={cn(switchVariant({ shape, size, permanent }), className)}
      {...props}
    >
